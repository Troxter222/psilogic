...
We evaluate PsiLogic against Adam, AdamW, and Lion using \textbf{FairBench}---a reproducible
...
bf16 AMP), PsiLogic records the best validation metric on \textbf{NLP perplexity} and
\textbf{ViT accuracy}, \textbf{beats Adam and numerically ties AdamW on ResNet}, and
\textbf{ties Adam/AdamW on diffusion}. Concretely: NLP perplexity $7.79 \pm 0.18$ vs.\
$8.17 \pm 0.08$ (AdamW, $p = 0.049$); ViT top-1 $0.244 \pm 0.006$ vs.\ $0.223 \pm 0.002$
(AdamW, $p = 0.015$); ResNet top-1 $0.222 \pm 0.001$ vs.\ Adam $0.172 \pm 0.004$
($p = 0.001$) and AdamW $0.219 \pm 0.005$ ($p = 0.44$, n.s.); diffusion MSE
$0.02009 \pm 0.00045$ vs.\ $0.01987 \pm 0.00006$ ($p = 0.49$, n.s.). Peak GPU memory is
comparable across optimizers; PsiLogic incurs \textbf{1.2--1.8$\times$} wall-clock overhead
on the June 2026 H100 run (\textbf{pre-Triton fusion}; Section~\ref{sec:limitations}).
Package v0.5+ ships an optional fused CUDA backend that does not change optimizer math; a
fused FairBench re-run is pending.
...
We propose \textbf{PsiLogic}, which adds a chaos-conditioned damping term to the Adam update.
...
We do \textbf{not} claim universal dominance over AdamW or Lion. We report limitations---including
...
without changing the global chaos signal. We write $\mathbf{g}_t = \nabla_\theta \mathcal{L}$ for the
...
\begin{equation}
  ...
  \label{eq:update}
\end{equation}
where $\hat{m}_t$ and $\hat{v}_t$ are the usual \emph{bias-corrected} Adam moments (we use $\hat{\cdot}$ consistently below and in Listing~\ref{lst:psilogic}) and $\delta_t$ is the
...
PsiLogic reports the best NLP perplexity and ViT accuracy in Table~\ref{tab:main},
beats Adam and numerically ties AdamW on ResNet, and ties Adam/AdamW on diffusion.
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  ...
  \item \textbf{Step-time overhead}---up to 1.79$\times$ vs.\ AdamW on ViT in the June 2026
    H100 reference (pre-fusion). The chaos detector tracks only scalar gradient statistics;
    the measured wall-clock gap is \emph{implementation-bound}. Package \textbf{v0.5+} adds
    an optional Triton fused CUDA step (\texttt{psilogic[cuda]}, \texttt{use\_fused\_cuda=True})
    that preserves optimizer math and targets $\leq 1.25\times$ AdamW step time on Ampere+;
    a fused FairBench H100 re-run is still pending.
  ...
  \item \textbf{Package default drift}---from v0.6 the bare constructor disables AGC / grad
    centralization; task presets and FairBench arena configs may still enable them. Record the
    exact preset when comparing to this paper.
\end{enumerate}
...
perplexity and ViT accuracy, beats Adam / ties AdamW on ResNet, ties on diffusion, and
reports wall-time overhead honestly (pre-fusion baseline). Future work: validate fused
overhead, increase seed count and training length, and seek independent replication at scale.
