...
def run_gc_agc_ablation():
    ...
    base_model = nn.Sequential(
        nn.Linear(10, 100), nn.GELU(), nn.Linear(100, 100), nn.GELU(), nn.Linear(100, 2)
    ).cuda()
    ...
    # Configurations
    train_config("PsiLogic (Full)", lambda m: PsiLogic(m.parameters(), lr=1e-3, chaos_warmup=0))
    train_config(
        "PsiLogic (No GC)",
    ...
    train_config(
        "AdamW Baseline", lambda m: torch.optim.AdamW(m.parameters(), lr=1e-3, weight_decay=1e-4)
    )
